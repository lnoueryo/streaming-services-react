...
export function useSignalingClient(url: string) {
  ...
  const connectPeer = async () => {
    ...
    newPc.ontrack = (e) => {
      ...
      e.track.onended = () => {
        ...
        setRemoteVideos((prev) =>
          prev.filter((v) => v.stream.id !== rStream.id)
        )
      }
  ...
  const scheduleTurnRefresh = (ttl: number) => {
    ...
    setTimeout(() => {
      console.log(
        '[TURN REFRESH] refreshing TURN credentials before expiration'
      )
